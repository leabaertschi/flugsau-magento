...
class UpgradeSchema implements UpgradeSchemaInterface {
	...
	public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context){
		...
		if (version_compare($context->getVersion(), '1.0.1') < 0) {
			/**
			 * Add column 'send_email' to 'customweb_twintcw_transaction'
			 */
			if ($installer->getConnection()->isTableExists($installer->getTable('customweb_twintcw_transaction'))) {
				$installer->getConnection()->addColumn($installer->getTable('customweb_twintcw_transaction'), 'send_email',
						[
		...
		if (version_compare($context->getVersion(), '1.0.4') < 0) {
			...
			$rows = $installer->getConnection()->fetchAll(
					"select * from {$installer->getTable('sales_sequence_meta')} where entity_type = 'twintcw_transaction'");
			foreach ($rows as $row) {
				$row['entity_type'] = 'twintcw_trx';
				$row['sequence_table'] = $this->getSequenceName('twintcw_trx', $row['store_id']);
		...
		if (version_compare($context->getVersion(), '1.0.5') < 0) {
			 ...
			 */
			$rows = $installer->getConnection()->fetchAll(
					"select * from {$installer->getTable('sales_sequence_meta')} where entity_type = 'twintcw_trx'");
			foreach ($rows as $row) {
				$row['entity_type'] = 'twicw_trx';
				$row['sequence_table'] = $this->getSequenceName('twicw_trx', $row['store_id']);
				$installer->getConnection()->update($installer->getTable('sales_sequence_meta'), $row, 'meta_id=' . $row['meta_id']);
			}

			foreach (array_keys($this->_storeManager->getStores(true)) as $storeId) {
				$oldTableName = $this->getSequenceName('twintcw_trx', $storeId);
				if ($installer->getConnection()->isTableExists($oldTableName)) {
					$newTableName = $this->getSequenceName('twicw_trx', $storeId);
					$installer->getConnection()->renameTable($oldTableName, $newTableName);
				}
			}
		}
