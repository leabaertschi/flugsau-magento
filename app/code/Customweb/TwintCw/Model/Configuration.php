...
class Configuration
{
	...
	{
		$modulesDirectory = $this->_moduleReader->getModuleDir(\Magento\Framework\Module\Dir::MODULE_ETC_DIR, 'Customweb_TwintCw');
		$this->_xmlParser->load($modulesDirectory . '/config.xml');
		$xmlArray = $this->_xmlParser->xmlToArray();
		foreach ($xmlArray['config']['_value']['default']['twintcw']['general'] as $key => $value) {
			if (is_array($value)) {
				$value = $value['_value'];
			}
	...
	{
		$mediaDirectory = $this->_storeManager->getStore()->getBaseMediaDir() . '/customweb/twintcw';
		$mediaUrl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) . '/customweb/twintcw';
