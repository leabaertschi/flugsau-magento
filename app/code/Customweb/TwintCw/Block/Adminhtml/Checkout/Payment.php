...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	...
	protected function _construct()
	{
		...
		$this->setId('twintcw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'twintcw-payment-submit-button');
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\TwintCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\TwintCw\Model\Authorization\Transaction) || !$transaction->getId()) {
